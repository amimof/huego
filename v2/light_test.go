...
func TestGetLight(t *testing.T) {
	...
	tests := []struct {
		...
		// 		on := false
		// 		l := &Light{On:&On{On:&on}}
		// 		return client.SetLight("63939343-2449-48b7-aa7d-5daea11dc546", l)
		// 	},
		// },
		// Return one light
		{
			...
			Error:      nil,
			Exec: func() (interface{}, error) {
				return client.GetLight("63939343-2449-48b7-aa7d-5daea11dc546")
			},
			Expect: &Light{
				Alert: &Alert{
	...
	for _, test := range tests {
		...
		if !reflect.DeepEqual(obj, test.Expect) {
			t.Fatalf("want: %+v\n, got: %+v", render.Render(test.Expect), render.Render(obj))
		}
	}
