...
func TestGetScene(t *testing.T) {
	...
	for _, test := range tests {
		...
		scene, err := client.GetScene("2f9ca1a6-7b96-4ac3-98cd-7a4c5be5bb30")
		if err != nil {
		...
		if !reflect.DeepEqual(scene, test.Expect) {
			t.Fatalf("want: %+v\n, got: %s", render.Render(test.Expect), render.Render(scene))
		}
	}
